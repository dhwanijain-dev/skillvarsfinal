...
function TechIcon({ name, position, orbitRadius, orbitSpeed, orbitOffset }: TechIconProps) {
        ...
        >
            ...
            {hovered && (
                <Html position={[0, 1, 0]} center>
                    <div className="bg-[#0A0A0A]/80 px-3 py-1 rounded-md text-white font-jetbrains text-sm">{name}</div>
                </Html>
            )}
        </mesh>
    )
...
export default function TechOrb() {
    ...
    const techIcons = TECH_STACK.map((tech, i) => {
